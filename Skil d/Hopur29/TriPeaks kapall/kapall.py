...
##    def quickGame(self):
##        for row in range(1,self.BOARDROWS):
...
##                 
    ...
    def animateCard(self):
        ...
        for i,triple in enumerate(self.animationQueue):
            ...
            distSq = dx**2 + dy**2
            card.moveTo(card.cardx + dx/1.5, card.cardy + dy/1.5)
            if distSq < 10:
                callback()
    ...
    def isGameOver(self):
        if self.game.hasWon():
            ...
            self.game.score += int(max(0, 3000 - self.game.finaltime*10))
            self.game.score += self.game.deckSize() * 100
            self.hasWon = True
    ...
    def drawBoard(self):
        ...
        helpStr = 'Press H to show/hide help'
        hintStr = 'Press S to show/hide legal cards'
        scoreStr = 'Press T to view the top 5 scores'
        ...
        scoreText = font.render(scoreStr, True, (255, 255, 255))
        DISPLAYSURF.blit(restartText, (710, 490))
        DISPLAYSURF.blit(quitText, (710, 510))
        DISPLAYSURF.blit(helpText, (710, 530))
        DISPLAYSURF.blit(hintText, (710, 550))
        DISPLAYSURF.blit(scoreText, (710, 570))
        ...
        if self.hasWon:
            ...
            DISPLAYSURF.blit(winText, (300, 300))
        ...
        if self.hasLost:
            ...
            DISPLAYSURF.blit(loseText, (300, 250))
            DISPLAYSURF.blit(againText, (200, 330))

        if self.showScore:
            self.renderHighScores()


    def renderHighScores(self):
        cx = self.WINDOWWIDTH/2 - 150
        cy= self.WINDOWHEIGHT/2 - 50

        self.highscoreRect = pygame.Rect(cx-40, cy-15, 340, 160)
        pygame.draw.rect(DISPLAYSURF, (0,0,0), self.highscoreRect)
        ...
        for row in self.scores:
            w = cx
            for col in row:
                DISPLAYSURF.blit(font.render(str(col)[0:6], True, (255, 255, 255)), (w, h))
                w += 80
            h += 15
    
        lineText = font.render(line, True, (255, 255, 255))
        tableText = font.render(table, True, (255, 255, 255))
        headerText = font.render(header, True, (255, 255, 255))
    
        DISPLAYSURF.blit(tableText, (cx + 100, cy))
        DISPLAYSURF.blit(headerText, (cx, cy + 20))
        DISPLAYSURF.blit(lineText, (cx, cy + 27))
        DISPLAYSURF.blit(lineText, (cx, cy + 107))


    def newHighScore(self):
        if self.game.score > self.scores[-1][1]:
            self.addHighScore()

    def addHighScore(self):
        font = pygame.font.SysFont("comicsansms", 35)
        DISPLAYSURF.blit(font.render("You got a new high score! Enter your name:", True, (255, 255, 255)), (150, 230))
        font = pygame.font.SysFont("comicsansms", 70)
        winStr = 'YOU WON!!!'
        winText = font.render(winStr, True, (0,255,0))
        DISPLAYSURF.blit(winText, (300, 300))
        playerName = str(inputbox.ask(DISPLAYSURF, 'Name:'))
                
        with open("highscores.csv", "w") as csvfile:
            a = csv.writer(csvfile, delimiter = ',')
            self.scores.append([playerName, self.game.score, self.game.finaltime, self.game.moves])
